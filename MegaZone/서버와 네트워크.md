...
C:\Users\Home\.ssh> scp -r folder root@192.168.56.120:/root/
-> 외부로 폴더 전송
C:\Users\Home\.ssh> scp -i id_rsa root@192.168.56.120:/root/.ssh/authorized_keys ./
-> 외부에서 파일 내려 받기 ./ 
...
DATABASE_DNS 172.31.0.102

Mobaxterm 설치 후 접속 설정
```
# cp authorized_keys authorized_keys.pub
# ssh-copy-id -i authorized_keys.pub root@172.31.0.100
# my-key.pem 파일 nat 컴퓨터에 옮기기
# chmod 400 my-key.pem  
 -> my-key.pem 권한 너무 높아서 읽기권한 뺴버리기
 # ssh -i my-key.pem root@172.31.0.101
```
192.168.35.226:221 -> 172.31.0.100:22 (WEB01)
192.168.35.226:222 -> 172.31.0.101:22 (WEB02)
192.168.35.226:223 -> 172.31.0.102:22 (DNS)

- 포트포워딩
```
# firewall-cmd --permanent --zone=external --add-forward-port=port=80:proto=tcp:toport=80:toaddr=10.0.0.1
# firewall-cmd --reload
```
## 3. DNS
